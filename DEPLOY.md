...
Бот, база и сайт живут на `216.57.108.105`, каталог `/opt/lordok_bot`,
домен lor-dok.ru. Адрес `5.42.101.251` из старых скриптов деплоя это
прежний сервер: там до сих пор отвечает nginx с копией лендинга и
просроченным сертификатом.

## Обновление до версии с паролями из окружения (август 2026)

На боевом сервере порты уже слушают только 127.0.0.1, а вот пароль базы
`lordok_secret` лежит в docker-compose.yml открытым текстом, и у Redis
пароля нет вовсе. Обновление требует ручных шагов, иначе бот не
поднимется.
